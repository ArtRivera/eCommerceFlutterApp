...
import 'package:ecommerce_app/src/features/authentication/presentation/sign_in/sign_in_controller.dart';
...
class _EmailPasswordSignInContentsState
  ...
  Future<void> _submit(EmailPasswordSignInState state) async {
    ...
    if (_formKey.currentState!.validate()) {
      final isSubmitSuccess = await ref
  ...
  Widget build(BuildContext context) {
    ref.listen(
        signInControllerProvider(widget.formType)
            .select((state) => state.value), (_, value) {
      if (value.hasError) {
        value.handleErrorMessageWithDialog(context);
      }
    });

    final _state = ref.watch(signInControllerProvider(widget.formType));
